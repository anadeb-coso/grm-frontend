  ...
  const pickDocument = async (hasImage = false) => {
    if (attachments.length < 3) {
      try {
        ...
        // if (result.type != "cancel" && localUri) { !result.canceled
        if (!result.canceled && localUri) {
          setLoading(true);
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : null}>
          ...
          <View style={{ flexDirection: 'row' }}>
            {attachments.length > 0 &&
              attachments.map((attachment, index) => (
                <ImageBackground
                  key={attachment.id}
                  source={(attachment.uri && attachment.uri.includes('.pdf')) ? require('../../../../../assets/pdf.png') : { uri: attachment.uri }}
                  style={{
                    height: 80,
                    width: 80,
                    marginHorizontal: 1,
          ...
          >
            ...
            <View style={styles.iconButtonStyle}>
              <IconButton
                ...
                // color={recording ? 'rgba(255, 1, 1, 1)' : '#707070'}
        ...
        {/* {recordingURI && (
          ...
          >
            <IconButton icon="play" iconColor={colors.primary} size={24} onPress={playSound} />
            <Text
              style={{
                fontFamily: 'Poppins_400Regular',
                fontSize: 12,
        ...
        {recordingURIs && recordingURIs.map((recording_url, index) => (
          ...
          >
            <IconButton icon={!soundOnPause && soundUrl == recording_url.uri ? "pause" : "play"} iconColor={colors.primary} size={24} onPress={
