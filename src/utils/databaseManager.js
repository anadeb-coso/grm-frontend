...
export const SyncToRemoteDatabase = async ({ username, password }, userEmail) => {
  const remoteDB = new PouchDB('http://134.122.71.147:5984/eadls', {
    skip_setup: true,
  });

  const grmRemoteDB = new PouchDB('http://134.122.71.147:5984/grm', {
    skip_setup: true,
  });

  const communesRemoteDB = new PouchDB('http://134.122.71.147:5984/eadls', {
    skip_setup: true,
  });
